...
namespace BayiPuan.MVCWebUI.Controllers
{
  [AuthorizationFilter]
    ...
    // GET: List
        ...
        {
            IGrid<Language> col = new Grid<Language>(_queryableRepository.Table.OrderByDescending(x => x.LanguageId).Skip((page - 1 ?? 0) * (rows ?? 10)).Take(rows ?? 10).AsNoTracking());
            col.Query = new NameValueCollection(Request.QueryString);

            if (col.Query != null)
            {
                col = new Grid<Language>(_queryableRepository.Table.OrderByDescending(x => x.LanguageId).AsNoTracking());
            }
            ...
            var total = _totalRowsRepository.Table.AsNoTracking().Where(x => x.TableName == "Languages").Select(x => x.TableRows).First();
