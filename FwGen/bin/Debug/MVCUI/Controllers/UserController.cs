...
namespace NewGenFramework.MvcWebUi.Controllers
{
    ...
    public class UserController : BaseController
    {
        ...
        {
            IGrid<User> col = new Grid<User>(_queryableRepository.Table.OrderByDescending(x => x.UserId).Skip((page - 1 ?? 0) * (rows ?? 10)).Take(rows ?? 10));
            col.Query = new NameValueCollection(Request.QueryString);

            if (col.Query != null)
            {
                col = new Grid<User>(_queryableRepository.Table.OrderByDescending(x => x.UserId));
            }
